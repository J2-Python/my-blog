...
)  # tags no va ayudar en la documentacion y que quede agrupado
...
async def create_post(
    body: Annotated[PostCreate, Depends(PostCreate.as_form)],
    image: Optional[UploadFile] = File(None),
    db: Session = Depends(get_db),
    user=Depends(get_current_user), # 👉 si lo voy a necesitar 2 veces a get_current_user
